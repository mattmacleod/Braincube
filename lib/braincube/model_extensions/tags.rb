    ...
    module Tags #:nodoc:
      ...
      module InstanceMethods
        ...
        def related
          ...
          taggable_ids = Tagging.where("taggable_id!=#{id}").where(:tag_id => tag_ids).where(:taggable_type => self.class.name).group("taggable_id, taggable_type").order("COUNT(taggable_id) DESC").map(&:taggable_id)
