...
(function($) {
	...
	$.fn.prettyPhoto = function(settings) {
		settings = jQuery.extend({
			...
			ajax_markup: '<div class="pp_ajax clearfix"></div>'
		}, settings);
			...
			$pp_pic_holder.fadeIn(function(){
				...
				switch(_getFileType(images[setPosition])){
					...
					case 'ajax':
						...
						ajax_url = images[setPosition];
						toInject = settings.ajax_markup;
						$.get(ajax_url, function(html){
							$pp_pic_holder.find('.pp_ajax').html(html);
							_setupAjaxButtons();
						})
					break;
				};
		...
		*/
		...
		function _setupAjaxButtons(){
			$(".close_lightbox").click(function(){
				$.prettyPhoto.close();
				return false;
			});
			
			$('.lightbox_submit').parents("form:first").ajaxForm({
				target: '.pp_ajax',
				beforeSerialize: _hideAjaxSubmitButton,
				complete: _getNextAjaxAction
			});
		};
		
		function _getNextAjaxAction(){
			if($("#lightbox_reload").length>0){
				window.location.reload();
			} else if( $("#lightbox_close").length>0 ){
				$.prettyPhoto.close();
			} else {
				_setupAjaxButtons();
			}
